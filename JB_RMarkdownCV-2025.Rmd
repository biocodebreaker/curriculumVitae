...
I have conducted a study on immune cell infiltration in the Gastric Cancer (GC) Tumor Microenvironment (TME) using the TCGA Gastric Cancer dataset. Gene expression data (mRNA RNA-Seq) of 448 Gastric Cancer (Stomach Adenocarcinoma) cases were downloaded from the Genomic Data Commons Portal TCGA-STAD project. 

The study aimed to identify Immune-Related Genes (IRGs) that have a significant impact on the survival of GC patients. These IRGs that can predict the prognosis of GC patients can act as biomarkers for predicting patient outcomes. They can also be targeted for cancer immunotherapy. 

Based on risk score calculation, we established a prognostic risk model comprising of eight differentially expressed Immune-Related Genes (IRGs) which were the best at predicting Overall Survival at the 1-year, 3-year and 5-year time points. 
...
The Multivariate Cox Proportional Hazards Model identified three (3) OS-Related DEGs (DSG2, JTB and PTTG1) as significant independent predictors of Overall Survival of Cervical Cancer patients. 

=> Generate a t-distributed Stochastic Neighbor Embedding (t-SNE) plot to visualize the distribution of the Tumor and Normal samples based on the expression of the 65 OS-Related DEGs.

TCGA, GTEx and GEO Datasets to be used in the Training cohort and validation cohort have been identified. They were divided into two cohorts: the Training Cohort and Validation cohort.

Training Cohort:  TCGA Primary tumors (306); TCGA-GTEx Normal (22); GEO (GSE39001, GSE63514).
Validation cohort: GSE44001 (300 tumor samples).
...
When I became Head of Department (HoD) in the Department of Medical Laboratory Sciences at KIU, I introduced a series of workshops and seminars to train postgraduate students and staff in the use of R for data analysis. 

# using  Posit-Cloud/RStudio + GitHub for Reproducible bioinformatics research. 
My major goal is to reproduce the work that is in the attached published article (Zhao et al., 2022; https://doi.org/10.3389/fgene.2022.1013571).

STEPS: Tutorial on Reproducibility of Bioinformatics Research using R and GitHub. 
1. We selected a PURELY BIOINFORMATICS recently published study ON CERVICAL Cancer from PubMed (https://doi.org/10.3389/fgene.2022.1013571). 
2. We opened a GitHub repository and named it "Reproduce-HMAG-Signature-Cervical-Cancer". 
3. We opened a README.md file and assigned the MIT License.
The README.md file contains Study Information, Datasets, required R packages and instructions on how to set up the R environment and install the required R packages. 
4. After setting up the GitHub Repository, we used the URL of the GitHub Repository (https://github.com/biocodebreaker/Reproduce-HMAG-Signature-Cervical-Cancer) to deploy the RStudio Project (16GB RAM, 4CPU). 

The study used gene expression and clinical data from three independent clinical cohorts: TCGA-CESC, GSE44001 and GSE52903.
...
=> Using PubMed parser to extract the metadata of the selected articles e.g. pmid, title, abstract, authors, journal, publication date, doi, etc.
=> Publishing the automated literature review using GitHub Pages for easy sharing and collaboration.

=> Using the Linux Terminal for data analysis tasks.
...
=> Developing a a FAIR-compliant, machine-readable, modern, data-driven GitHub-hosted project website powered by RStudio, which can be easily updated when the underlying data in the RStudio project changes.

=> Developing end-to-end data analysis workflows using R and GitHub.

=> Using R Markdown and Quarto for publishing reproducible research reports and data-driven manuscripts e.g. PapersWithCode. 

# My GitHub skills:
=> Using the proper branch master vs main to ensure code is correctly committed to the right branch.
=> using the Personal Access Token instead of the Password to ensure secure access to the GitHub repository.


=> Using Linux bash/shell scripting for automation of data analysis tasks, e.g. GNU parallel for parallel processing of multiple files.

=> Using R Bioconductor or CRAN (Comprehensive R Archive Network) packages for data analysis. 

=> saving R objects to disk using the saveRDS() function and loading them using the readRDS() function for easy sharing with collaborators to ensure reproducibility.
=> saving the data to an RData file using the save() function and loading it using the load() function to maintain a consistent dataset that can be reused across projects and platforms. 

=> committing my work to a GitHub repository using the git version control system.
=> Using sessionInfo() to record the R version and the versions of the R packages used in the analysis for reproducibility.
...
=> Generating a a flow diagram (flow chart) showing the steps of our analysis. 
=> Usimg the TCGAbiolinks R package for data collection and preprocessing of TCGA data.
=> Using the GEOquery R package for data collection and preprocessing of GEO data.

=> Determining overall survival (OS) and disease-free survival (DFS) using TCGA and GEO data. 
=> Removal of Batch Effects using R packages like sva, ComBat and merging the cohorts after batch effect removal.
=> Using the the Molecular Signatures Database (MSigDB) for ontology analysis.
=> Gene Set Variation Analysis (GSVA) using the GSVA package R package.
=> Using the limma R package for Differentially Expressed Genes (DEGs) analysis.
...
=> Using the survival R package for survival analysis.
=> Using the ggplot2 R package for data visualization.
=> Univariate and multivariate Cox regression analysis using the survival R package.
=> Receiver Operating Characteristic (ROC) curve analysis.
=> Using the glmnet R package for LASSO regression analysis.


the R code to reproduce the analysis was encapsulated in a yaml configuration file such that we have the input files/data (e.g. Tables and Figures) and output files/results were well documented. Snakemake was used to run the workflow. 

Reproducibility is now one of the most desirable tenets of academic/scientific publishing yet authors typically do not provide the code alongside their manuscripts. I am currently spearheading Reproducible Bioinformatics in the Department of Medical Laboratory Sciences at Kampala International University. 

As Head of Department, I am training my postgraduate students to reproduce the paper published on Cervical Cancer so that they appreciate the importance of having reproducible workflows and providing code alongside the manuscript so that others can build on it. This work can be found here: [https://github.com/biocodebreaker/Reproduce-HMAG-Signature-Cervical-Cancer]. 
...
# current experiences and future plans for knowledge sharing and community building
...
Our work inherently implements the FAIR Guiding Principles of Findability, Accessibility, Interoperability and Reusability [see Link 3]. We shall distribute the Research Outputs from our projects via preprint servers, and other tools like Zenodo, Figshare for easier Accessibility and Reusability.
...
I also introduced the use of R Markdown for reproducible research. This initiative has been well received and has led to a significant increase in the use of R for data analysis in the department.
